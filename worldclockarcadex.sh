...
if  test $OFF_NOW -gt $OFF_1 && (( $toggledst % 2 != 0 )) ||  test $OFF_NOW -gt $OFF_7 && (( $toggledst % 2 != 0 ))
then
...
spacessub=$(printf "%*s%s" "$addspacessub" "")  
    ...
    echo -e "   ${rw}| ${lb}${airports[$var]}  $spacessub| ${rr}$(TZ=$tz date '+%z') ${rw}| ${lo}$(TZ=$tz date '+%a %d-%b, %Y') ${rw}| ${lg}$(TZ=$tz date '+%I:%M %p') ${rw}|"
    echo -e "   --------------------------------------------------------- $spacessub"
    let "var=var+1"
    done
    var=0
